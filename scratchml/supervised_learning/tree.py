...
class NodeC():
  """
  A class representing a node of a decision tree classifier model.
  """
...
class NodeR():
  """
  A class representing a node of a decision tree regressor model.
  """
  def __init__(self, error = 0, num_samples = 0, threshold = None,
               predicted_value = None, feature_index = None,left = None, right = None):
    self.error = error
    self.num_samples = num_samples
...
class DecisionTreeR():
  ...
  def _grow_tree(self,X,y,depth=0): 
    # Base node
    node = NodeR (
      error = self._calculate_error(y),
      num_samples = len(y),
      predicted_value = np.average(y)
    )
    ...
    # Check if either side is too small. If so, reutrn a leaf node and don't split
    ...
    if self.splitter == 'best':
      ...
      for idx in features_to_consider:
        ...
        for threshold in possible_thresholds:
          ...
          # Calculate the weighted impurity for the split
          left_error = self._calculate_error(y_left)
          right_error = self._calculate_error(y_right)
  ...
  def _mse(self,y):
    ...
    mse = np.mean((y - pred_value)**2)
    return mse
  
  def _friedman_mse(self,y):
    pred_value = np.mean(y)
    total_variance = np.sum((y - pred_value) ** 2)

    unique_values, counts = np.unique(y, return_counts=True)

    weighted_variance = 0
    for value, count in zip(unique_values,counts):
      weighted_variance += (count / len(y)) * (value - pred_value) ** 2
    
    friedman_mse = (total_variance - weighted_variance) / len(y)

    return friedman_mse
  
  def _mae(self,y):
    median_value = np.median(y)
    mae = np.mean(np.abs(y - median_value))
    return mae
